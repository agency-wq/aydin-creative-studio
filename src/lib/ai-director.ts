...
function buildSystemPrompt(): string {
  return [
    ...
    "",
    "## REGOLE B-ROLL — CRITICHE",
    "",
    "6) DIVIETO ASSOLUTO DI PERSONE NEL B-ROLL. Le query b-roll devono mostrare SOLO:",
    "   - Oggetti e prodotti in close-up (es. 'laptop keyboard close up', 'coffee cup steam morning')",
    "   - Ambienti vuoti (es. 'empty modern office interior', 'kitchen counter clean')",
    "   - Dettagli/texture (es. 'clock mechanism gears macro', 'water drops slow motion')",
    "   - Paesaggi e cityscape (es. 'italian city aerial sunset', 'mediterranean architecture')",
    "   - Mani/dettagli corpo senza volto (es. 'hands typing laptop close up', 'hand writing notebook')",
    "   - Astratto/motion (es. 'abstract light particles', 'bokeh lights night city')",
    "   MAI query che possano includere persone visibili: NO 'man', 'woman', 'person', 'people', 'team', 'doctor', 'business', 'professional' come soggetto. Se il concetto richiede una persona, usa l'oggetto o l'ambiente invece (es. 'stethoscope medical desk' invece di 'doctor office').",
    "7) Query Pexels: INGLESE, specifiche, 4-7 parole. NESSUN tag extra, solo le keyword di ricerca.",
    "8) Fallback: 2-3 query più generiche, SEMPRE senza persone. Preferisci close-up di oggetti e ambienti vuoti.",
    "",
    "## MUSICA",
    "",
...
function buildUserPrompt(input: DirectorInput, target: number): string {
  ...
  return [
    ...
    "  ],",
    '  "broll": [',
    '    { "startMs": 9100, "endMs": 12500, "query": "modern office desk laptop close up", "fallbackQueries": ["workspace keyboard coffee aerial", "laptop screen hands typing close up"], "reason": "contesto lavorativo — solo oggetti, no persone" }',
    "  ],",
    '  "music": {',
